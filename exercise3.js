...
if (userInput !== null && saveButton !== null && resultEl !== null) {
  ...
  saveButton.addEventListener("click", function(event) {
    ...
    if (submitted !== "") {
      ...
      // Erase user's text from the textarea with ''
      userInput.value = "";
    }
  ...
  //Pass in event as a parameter, you will reference it later in the function when adding preventDefault behaviour
  resultEl.addEventListener("click", function(event) {
    // Reference to the thing that was just clicked
    var clicked = event.target;

    // Check if it is a delete button (what they share is the item-delete class)
